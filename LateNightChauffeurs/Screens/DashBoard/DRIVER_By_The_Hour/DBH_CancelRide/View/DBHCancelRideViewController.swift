...
extension DBHCancelRideViewController {
    ...
    func cancelFutureRideAPI() {
        ...
        self.viewModel.requestForcancelDBHRideAPIServices(perams: perams) { success, model, error in
    ...
    func CancelRideAmount() {
        ...
        let perams = [ "ride_id":str_RideID,
                       "cancel_time":currentDate]
        self.viewModel.requestForCancelDBHRideAmountAPIServices(perams: perams) { success, model, error in
            if success, let UserData = model {
                DispatchQueue.main.async { [self] in
                    indicator.hideActivityIndicator()
                    if UserData.loginStatus == "1" {
