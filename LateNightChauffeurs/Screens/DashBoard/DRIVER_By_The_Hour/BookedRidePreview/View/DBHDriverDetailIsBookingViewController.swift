//
//  DBHDriverDetailIsBookingViewController.swift
//  LateNightChauffeurs
//
...
class DBHDriverDetailIsBookingViewController: UIViewController {
    ...
    lazy var viewModel = {
        DBHDriverDetailIBookingViewModel()
    }()
...
extension DBHDriverDetailIsBookingViewController {
    //Ride Details get
...
//       let perams = [ "user_id":str_UserID,"ride_id":self.str_FutureRideID]
        let perams = [ "user_id":str_UserID,"ride_id":self.str_FutureRideID]
        self.viewModel.requestForDriverDetailInDBHBookingAPIServices(perams: perams) { success, model, error in
