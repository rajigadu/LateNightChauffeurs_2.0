...
class SmallBannerAdsVC: UIViewController {
    
    @IBOutlet weak var btn_SmallBannerADsRef: UIButton!
    var str_TodayDate = ""
        ...
        self.viewModel.requestForbannerServices(perams: ["currentdate":str_CurrentDate,"userid":str_userID,"devicetoken":FCMDeviceToken,"device_type": "ios","app_version": str_AppVersion]) { success, model, error in
            if success, let UserData = model {
                DispatchQueue.main.async { [self] in
                    ...
                    self.btn_SmallBannerADsRef.setImage(img, for: .normal)
                    timerForBannerADs = Timer.scheduledTimer(
                        ...
                        userInfo: nil,
                        repeats: true)
                    self.btn_SmallBannerADsRef.isUserInteractionEnabled = true
                }
            } else {
                DispatchQueue.main.async { [self] in
                    indicator.hideActivityIndicator()
                    self.btn_SmallBannerADsRef.setImage(UIImage(named: "appLogo"), for: .normal)
                }
            }
        }
    ...
    @objc func handleTimerBannerADs(sender: UIButton) {
        if curCheckingStateForBanners < ary_BannerAdsList.count {
            ...
            if profileURL == "" {
                ...
                {
                    self.btn_SmallBannerADsRef.addTarget(self, action: #selector(self.loadingwebUrlBasedselectedBannerADClicked), for: .touchUpInside)
                }
            } else {
                if let url = URL(string: finalImageURL) {
                    do {
                        if let datastr = try Data(contentsOf: url) as? Data {
                            ...
                            DispatchQueue.main.async
                            {
                                self.btn_SmallBannerADsRef.setBackgroundImage(img, for: .normal)
                                self.btn_SmallBannerADsRef.addTarget(self, action: #selector(self.loadingwebUrlBasedselectedBannerADClicked), for: .touchUpInside)
                            }
                        }
                    } catch  {
