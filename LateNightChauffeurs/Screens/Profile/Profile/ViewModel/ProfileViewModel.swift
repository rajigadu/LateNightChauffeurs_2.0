...
class ProfileViewModel: NSObject {
    ...
    func requestForEditProfileServices(perams: Dictionary<String,String>, picImage: UIImage, fileName: String, completion: @escaping (Bool, ProfileUserData?, String?) -> ()) {
        ProfileServices.requestForEditProfileServices(perams, picImage: picImage, fileName: fileName) { success, model, error in
            if success, let EditProfileUserData = model {
                if EditProfileUserData.loginStatus == "1" {
                    ...
                    UserDefaults.standard.set(EditProfileUserData.userData?[0].profilePic, forKey: "userProfilepic")
